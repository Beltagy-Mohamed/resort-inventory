...
def inventory_report(request):
    ...
    # Annotate total value for each product (price * quantity)
    products = products.annotate(total_value=F("price") * F("quantity")).order_by("-id")

    # Evaluate once â€” count(), aggregates and the template's own loop
    # would otherwise each re-run the query from scratch.
    ...
    if request.GET.get("export") == "xlsx":
        ...
        headers = ['ظƒظˆط¯ ط§ظ„ظ…ظ†طھط¬', 'ط§ظ„ط§ط³ظ…', 'ط§ظ„طھطµظ†ظٹظپ', 'ط§ظ„ط³ط¹ط±', 'ط§ظ„ظƒظ…ظٹط©', 'ط§ظ„ط­ط¯ ط§ظ„ط£ط¯ظ†ظ‰', 'ط§ظ„ط­ط§ظ„ط©', 'ط§ظ„ظ‚ظٹظ…ط© ط§ظ„ط¥ط¬ظ…ط§ظ„ظٹط©']
        ...
        ws.column_dimensions['B'].width = 35 # Name column wider
        ...
        for row_num, p in enumerate(products_list, 2):
            row_data = [
                p.product_code,
                p.name,
                p.category.name if p.category else 'ط¨ط¯ظˆظ† طھطµظ†ظٹظپ',
                float(p.price),
                p.quantity,
                p.minimum_stock,
                p.stock_status_text,
            ...
            for col_num, value in enumerate(row_data, 1):
                ...
                cell.border = thin_border
                if col_num in [4, 8]: # Price and Total Value
                    cell.number_format = '#,##0.00'
                    cell.alignment = center_align
                elif col_num in [5, 6]: # Quantity and Min Stock
                    cell.alignment = center_align
                else:
