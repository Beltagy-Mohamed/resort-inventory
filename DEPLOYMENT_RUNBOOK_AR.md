# دليل التشغيل والنشر الإنتاجي — Resort Inventory

هذا الدليل يوصي بـ **Ubuntu VPS + Gunicorn + Nginx + PostgreSQL**. لا تستخدم Vercel لهذه النسخة: التطبيق Django stateful، ويولّد ملفات QR/Barcode في التخزين المحلي، كما لا توجد تهيئة Vercel أو قاعدة بيانات خارجية ضمن المشروع.
...
الـ media المحلي مقبول فقط على VPS واحد مع backup. للتوسع أو تعدد الخوادم، انقل QR/Barcode إلى S3/R2/Cloudinary وغير storage backend قبل النشر.
...
- إضافة منتج، مع رمز فريد وQR وBarcode ظاهرين وقابلين للطباعة.
...
انسخ media أيضاً إذا بقي التخزين محلياً؛ قاعدة البيانات وحدها لا تكفي لاستعادة QR/Barcode. يمكن إعادة توليدها، لكن توثيق هذا القرار ضروري.
